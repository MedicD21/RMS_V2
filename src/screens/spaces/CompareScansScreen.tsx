  ...
  if (!space || scans.length < 2) {
    return (
      <div className="flex items-center justify-center h-full" style={{ background: 'var(--bg)' }}>
        <p style={{ color: 'var(--text-secondary)' }}>Need at least 2 scans to compare.</p>
      </div>
    )
  }

  const scoreDelta = rightScan && leftScan ? rightScan.score - leftScan.score : 0
  const deltaColor = scoreDelta > 0 ? 'var(--score-high)' : scoreDelta < 0 ? 'var(--score-low)' : 'var(--text-secondary)'
      ...
      <div className="scroll-area flex-1 px-5 pb-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div key={label}>
              <p className="text-xs font-semibold mb-1.5 text-center" style={{ color: 'var(--text-muted)' }}>{label}</p>
              <select
                value={idx}
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-xs text-center" style={{ color: 'var(--text-muted)', maxWidth: 80 }}>
            ...
            <div className="flex flex-col items-center gap-1">
              ...
              <span className="text-xs font-medium" style={{ color: 'var(--text-muted)' }}>pts</span>
            </div>
        ...
        {allMetricLabels.length > 0 && (
          ...
          >
            ...
            {allMetricLabels.map((label) => {
              const lScore = leftScan?.breakdownMetrics.find((m) => m.label === label)?.score ?? 0
              const rScore = rightScan?.breakdownMetrics.find((m) => m.label === label)?.score ?? 0
              const delta = rScore - lScore
              const deltaC =
                delta > 0 ? 'var(--score-high)' : delta < 0 ? 'var(--score-low)' : 'var(--text-muted)'

              return (
                <div key={label} className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-semibold" style={{ color: 'var(--text-secondary)' }}>{label}</span>
                    <span className="text-xs font-bold" style={{ color: deltaC }}>
